...
- validated ordered capability `registry_digest`; and
...
clarification, material-parameter transit, approval binding, and verified
business answer gates. It must record an independently verified exact
Pi/Pi-Bridge/provider/model/prompt/tool/runtime binding and
`runtime_evidence_verified:true`. Every executed scenario must contain the raw
Broker exchange verified with receipt/approval keys loaded through the
root-managed trusted-authority configuration. The capture's manifest and
package digests must equal the current release, and the report
`registry_digest` must equal the routed release's validated ordered capability
digest. This digest is not the raw SHA-256 of the outer registry JSON document.
Corpus tests or normalized self-reported events alone do not prove Pi
end-to-end success, and a passing report from an older release or registry must
not be reused.
...
`--expected-capture-binding` file, with `--trusted-authority-config` pointing
to the root-managed exact-release trust chain. This earlier check is also
offline and read-only, and catches trace/signature/receipt/approval/release
mismatches before the scorer report is created.
...
the committed event stream during Pi `session_shutdown`, and the parent
requires clean EOF, child exit zero, and an exact action/capability one-event
match for the bounded canonical final answer. Registry discovery can support a
clarification/refusal but cannot satisfy business success; diagnostics must
remain `business_succeeded:false` because their receipt proves the query rather
than the inspected operation's success. Exercise missing commit, truncated stream,
wrong digest, duplicate receipt/operation, extra stdout, unsupported success,
and clarification/refusal-with-business-evidence negatives. This protects the
returned answer; it does not replace the retained full Pi scenario trace,
95%-selection gate, real Odoo receipts, or financial-standard-answer evidence.
